...
■ 물환경보전법 제38조의2: 수질자동측정기기 부착 의무 대상 규정
■ 물환경보전법 제38조의3: 금지행위 및 운영·관리기준 준수 의무 (정도검사 간접 근거)
...
■ 정도검사 주기(2013년 이후 기기, 별표7): TOC·TN·TP·SS·COD·DO·BOD — 1차2년·2차2년·3차이후1년 / pH·TU·CL — 모두2년
■ 정도검사 주기(2013년 이전 기기, 별표7의1): TOC·TN·TP·SS·COD·DO·BOD — 1년 / pH·TU·CL — 2년
■ COD 현장적용계수 기준: 15% 또는 3.0mg/L 이하 (둘 다 충족해야 적합)
■ pH 현장적용계수 기준: ±0.20pH 이내 (절대값)
...
[답변 구조 가이드 — 질문 유형별]
■ 기준값·수치 질문 (반복성·드리프트·직선성·주기·수수료):
  → 항목별 수치를 간결한 목록 또는 표로 제시 → 고시 별표1의1 또는 시행규칙 별표13 명시
■ 절차 질문 (형식승인·정도검사 신청 방법):
  → 단계별 번호 목록(1단계→2단계) → 각 단계의 담당 기관 명시
■ 법적 의무 질문 (받아야 하나요·위반 시):
  → 근거 조문 먼저 → 핵심 해석 → 위반 시 제재 순서
■ 비교 질문 (수질TMS vs 먹는물·형식승인 vs 정도검사):
  → 비교 표 또는 핵심 차이점 2~3가지 나열
■ 계산 방법 질문:
  → 공식 먼저 → 변수 설명 → 계산 과정 → **합격 기준과 비교하여 적합/부적합 판정까지 명시** → 예시 순서
■ 수치가 주어진 계산 질문 (측정값 제시된 경우):
  → 계산 결과를 구한 뒤 반드시 "→ 합격 기준 X% 이하이므로 [적합/부적합]" 판정 문장으로 마무리`;
